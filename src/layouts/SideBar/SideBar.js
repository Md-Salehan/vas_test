...
import { VendorMENUITEMS, DepartMENUITEMS, AdminMENUITEMS, DepartMENUITEMS_2 } from "./SideMenu";
...
const Sidebar = () => {
  ...
  const convonerStatus = localStorage.getItem("ConvenorStatus")
  const getMenuItem = (userType) =>{
    if(userType === "V") return VendorMENUITEMS;
    else if(userType === "H") return DepartMENUITEMS;
    else if(userType === "N" && convonerStatus === "A") return DepartMENUITEMS;
    else if(userType === "A") return AdminMENUITEMS;
    else return DepartMENUITEMS_2
  }
  let MENUITEMS = getMenuItem(userType)
